...
uint16_t encrypt_to_ciphertext(char * msg, uint16_t msgLen, byte iv[],char ciphertext[]) {
...
uint16_t decrypt_to_cleartext(byte msg[], uint16_t msgLen, byte iv[],char cleartext[]) {
...
void loop() {
  ...
  uint16_t encLen = encrypt_to_ciphertext((char*)cleartext, msgLen, enc_iv, ciphertext);
  ...
  uint16_t decLen = decrypt_to_cleartext(base64decoded, strlen((char*)base64decoded), enc_iv, cleartext);
